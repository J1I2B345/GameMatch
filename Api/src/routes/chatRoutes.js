...
router.get("/getUsersToChat/:_id", async (req, res) => {
	...
	try {
		...
		let chats = user[0].usersToChat.map((e) => {
			return { _id: e._id, username: e.username, rating: e.rating, img: e.img };
		});
...
router.get("/", async (req, res) => {
	try {
		...
		const projectMessages = chats.map((e) => {
			return {
...
router.post("/", async (req, res) => {
	try {
		...
		if (chatSaved) {
			const addUser1ToChatOfUser0 = await User.findByIdAndUpdate(
				users[0],
				{
			...
			const addUser0ToChatOfUser1 = await User.findByIdAndUpdate(
				users[1],
				{
...
router.delete("/message/:id", async (req, res) => {
	try {
		...
		if (messageDeleted)
			...
			throw new Error(
				"message not deleted -probably couldnt be found because it was already deleted, try another-"
			);
	} catch (e) {
		res.status(400).json({ error: e.message });
	}
...
router.delete("/users", async (req, res) => {
	try {
		...
		if (chatDeleted.deletedCount > 0) {
			...
			return res.json({
				message: `chat succesfully deleted ${chatDeleted.deletedCount}`,
			});
		} else {
